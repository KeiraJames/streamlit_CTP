...
def get_ring_html_css():
    ...
    .home-tab-content {{ background-color: #F0FFF0; padding: 15px; border-radius: 8px; }}
    </style>"""
...
def send_message(messages):
    if not GEMINI_API_KEY or GEMINI_API_KEY == "your_gemini_api_key_here": return "Gemini API Key is not configured."
    ...
    try:
        ...
        return "Sorry, received an unexpected response."
    except requests.exceptions.Timeout: return "Sorry, the request timed out."
    except requests.exceptions.RequestException as e:
        err_detail = "";
        if hasattr(e, 'response') and e.response is not None:
            try: err_detail = e.response.json().get('error', {}).get('message', e.response.text)
            except: err_detail = e.response.text
        return f"Sorry, API error: {err_detail or str(e)}"
    except Exception as e: return f"Oops, chat error: {e}"
...
def find_care_instructions(plant_name_id, care_data, threshold=75): # Renamed match_threshold
    ...
    for p in care_data: # Exact matches
    ...
    best_match, high_score = None, 0 # Fuzzy
...
def find_similar_plant_matches(id_r, care_data, limit=3, score_thresh=60): # Shortened var names
...
def main():
    ...
    if st.session_state.current_nav_choice == "🏠 Home":
        st.markdown("<div class='home-tab-content'>", unsafe_allow_html=True) # Apply background
        ...
        if st.session_state.saved_photos:
            ...
            if recent:
                cols_home = st.columns(len(recent))
                for i, (nick, p_data) in enumerate(recent):
                    with cols_home[i]:
                        with st.container(border=True, height=280): # Reverted to st.image
        ...
        st.markdown("</div>", unsafe_allow_html=True) # Close home-tab-content div
            ...
            if not st.session_state.plant_id_result:
                ...
                if st.session_state.saving_mode:
                    ...
                    with st.form("save_form_id"):
                        ...
                        if st.form_submit_button("✅ Confirm"):
                            if not nick: st.warning("Nickname needed.")
                                ...
                                for k_reset in ["plant_id_result","plant_care_info","suggestions","uploaded_file_bytes","uploaded_file_type","chat_history","saving_mode","plant_id_result_for_care_check","suggestion_just_selected","current_chatbot_plant_name"]: st.session_state[k_reset] = [] if k_reset=="chat_history" else None
                    ...
                    if 'error' not in curr_id_res:
                        ...
                        else: # No specific care
                            st.warning("No specific care instructions found.")
                            if st.session_state.suggestions is None: st.session_state.suggestions=find_similar_plant_matches(curr_id_res,care_data); st.rerun()
        ...
        if not st.session_state.saved_photos: st.info("No plants saved yet. Go to 'Identify New Plant'.")
            ...
            if st.session_state[confirm_key]:
                ...
                if c1d.button("Yes, Delete It",key=f"yes_del_final_{nick_to_view}",type="primary"):
                    ...
                    for k_rst in ["plant_id_result","plant_care_info","suggestions","chat_history","viewing_plant_stats","current_chatbot_plant_name"]: st.session_state[k_rst]=[] if k_rst=="chat_history" else None
...
if __name__ == "__main__":
    if not PLANTNET_API_KEY or PLANTNET_API_KEY == "your_plantnet_api_key_here": st.sidebar.warning("PlantNet Key missing. Demo mode.",icon="🔑")
    if not GEMINI_API_KEY or GEMINI_API_KEY == "your_gemini_api_key_here": st.sidebar.warning("Gemini Key missing. Chat limited.",icon="🔑")
    main()
